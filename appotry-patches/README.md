...
| `scripts/patch-linux-window-ui.test.js` | `scripts/patch-linux-window-ui.test.js` | 对应测试（含 sparkleManager 修复用例） |
...
sync-upstream.yml (schedule 每 6h)
    ├─ reset --hard 到上游 main（丢弃旧补丁提交）
    ├─ apply.sh 重新应用补丁
    ├─ commit（chore: apply appotry patches on upstream <SHA>）
    └─ push --force-with-lease releases
            │
            ▼
...
分支策略：**reset-based**。releases 永远等于 上游 main + 补丁，不使用
merge，避免历史与上游无限分叉（之前 225 ahead 的教训）。
